...
bool Json::is_number() const noexcept { return is_int() || is_double(); }
...
class Parser {
 public:
  Parser(std::string_view input, Json::ParseLimits limits) : input_(input), limits_(limits) {}
...
void stringify_into(std::string& out, const Json& j) {
