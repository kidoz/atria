...
[[nodiscard]] std::expected<SocketHandle, SocketError> listen_tcp(std::string_view host,
                                                                   std::uint16_t port,
                                                                   int backlog);

[[nodiscard]] std::expected<SocketHandle, SocketError> connect_tcp(std::string_view host,
                                                                    std::uint16_t port);
...
// connection limits.
[[nodiscard]] std::expected<SocketHandle, SocketError> accept_connection_with_peer(
    SocketHandle& listener, std::string& peer_ip);

[[nodiscard]] std::expected<std::size_t, SocketError> recv_some(SocketHandle& sock, char* buf,
                                                                 std::size_t cap);

[[nodiscard]] std::expected<std::size_t, SocketError> send_all(SocketHandle& sock,
...
[[nodiscard]] std::expected<std::size_t, SocketError> send_some(SocketHandle& sock,
                                                                 std::string_view data);

// I/O timeouts via SO_RCVTIMEO / SO_SNDTIMEO. A value of 0 disables the timeout.
[[nodiscard]] std::expected<void, SocketError> set_recv_timeout(SocketHandle& sock,
                                                                 std::uint32_t milliseconds);

[[nodiscard]] std::expected<void, SocketError> set_send_timeout(SocketHandle& sock,
                                                                 std::uint32_t milliseconds);
