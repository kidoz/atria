...
Application& Application::group(std::string_view prefix,
                                const std::function<void(RouteGroup&)>& builder) {
...
void Application::shutdown() noexcept { running_.store(false); }
...
[[nodiscard]] Json build_operation_object(const RouteMeta& meta) {
  ...
  if (meta.request_body.has_value()) {
    Json::Object content;
    content.emplace_back(meta.request_body->content_type,
                          Json::object({{"schema", meta.request_body->schema}}));
    operation.emplace_back(
        "requestBody",
        Json::object({{"required", meta.request_body->required},
                      {"content", Json{std::move(content)}}}));
  }
  ...
  if (meta.responses.empty()) {
    ...
    for (const auto& response : meta.responses) {
      ...
      response_obj.emplace_back("description", response.description.empty()
                                                    ? std::string{""}
                                                    : response.description);
      if (response.schema.has_value() && !response.content_type.empty()) {
        Json::Object content;
        content.emplace_back(response.content_type,
                              Json::object({{"schema", *response.schema}}));
        response_obj.emplace_back("content", Json{std::move(content)});
      }
      responses.emplace_back(std::to_string(response.status_code),
                              Json{std::move(response_obj)});
    }
  }
...
Json Application::openapi_json() const {
  ...
  root.emplace_back("info", Json::object({
  ...
  router_.for_each_route([&](const RouteMeta& meta) {
    auto entry = std::find_if(
        path_entries.begin(), path_entries.end(),
        [&](const auto& candidate) { return candidate.first == meta.path; });
